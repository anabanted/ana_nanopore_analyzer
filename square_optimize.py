...
class D:
    ...
    def optimize_k(self, a: "A", k):
        ...
        error_list = np.array(
            [
                self.error(data, self.step(a[k - 1], a[k], start, end, dk))
                for dk in dk_list
            ]
        )
...
class A:
    def __init__(self, d: D, data, K):
        self.li = np.array(
            [self.mean(data[d[i]: d[i + 1]], data[d[i]]) for i in range(K)]
        )
...
class Model:
    ...
    def optimize(self):
        models = [ModelKTotalData(self.data, K)
...
class EventFinder:
    ...
    def baseline(self):
        return np.concatenate(
            [
                np.array(self.before_event()["values"]),
                np.array(self.after_event()["values"]),
            ]
        ).mean()
...
class ModelKTotalData:
    ...
    def baseline(self):
        return np.concatenate(
            self.before_event.data_ndarray, self.after_event.data_ndarray
        ).mean()

    def model(self):
        return np.concatenate(
            [self.before_event.model(), self.event.model(),
             self.after_event.model()]
        )
